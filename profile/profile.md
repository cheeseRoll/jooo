# Candidate Profile & Scoring Rubric — Harsh Belde

This file is the single source of truth for what "a good job" means in this system.
The nightly scoring session reads this file before scoring any job.
...
## What he brings (use these when judging fit)

- **Big Four audit** across FMCG, manufacturing, listed fintech — profitability analysis,
  variance & margin modeling, benchmarking, insights delivered to senior leadership
- **Financial modelling** — integrated 3-statement modelling, unit economics, scenario &
  sensitivity analysis. Showcase: **Third Wave Coffee NSO & unit-economics model** (Excel +
  Power BI): store-level breakeven, NSO payback/IRR, cohort cash burn, portfolio-level peak
  capital need — a genuinely FP&A-grade artifact
- **FP&A core skills**: budgeting & forecasting, variance analysis, corporate performance
  management, Ind AS, advanced Excel, Power BI, Python automation (built an AI-assisted
...
Enter a **Strategic Finance / Business Finance / FP&A / Corporate Finance / Founder's
Office (finance)** role at a startup where he can contribute to strategy from a strong
finance base, learn how a startup operates end-to-end, and eventually build something
himself. Learning velocity and proximity to decision-makers matter more than brand or a
small salary delta.

## Hard criteria (prefilter — jobs failing these are dropped before scoring)

1. **Role family**: FP&A, financial planning, strategic finance, business finance, corporate
   finance, finance analyst/manager, founder's office (finance), chief of staff (finance-heavy),
   revenue/unit-economics analyst, **financial modelling / 3-statement modelling** roles.
...
| Signal | Weight | What scores high |
|---|---|---|
...
<50 = shown only in the "everything else" list.

## Per-job outputs the scorer must produce

- `fit_score` (0–100), `band`
- `geo_tag`: `BLR` | `ABROAD+VISA` | `ABROAD?VISA`
- `stage`: funding stage + last raise if known (web-check unknown companies)
- `why` — 2 lines max, specific, no fluff
- `resume_tips` — wording/emphasis tweaks for THIS job (e.g. "lead with the TWC model,
  rename 'Corporate performance analysis' bullet to mirror their 'business performance
  reporting' phrasing"). **Never fabricate experience.**
- `red_flags` — empty if none

## Cover letter voice (used post-Applied)

First person, confident but not salesy, concrete numbers from his experience, one line on
